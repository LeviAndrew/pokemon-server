...
export class User extends BasicManager {
  ...
  private async login(msg) {
    ...
    let ret: any = await this.model
      ...
      .select(msg.data.success.queryObject.select)
      .lean()
      .exec();
    if (ret.length === 1) {
      let userRet: any = ret[0];
      if (!await this.comparePassword({userPassword, hashPassword: userRet.password})) {
  ...
  private async verifyPassword(msg) {
    ...
    try {
      ...
      let compare = await this.comparePassword({userPassword: msg.data.success.password, hashPassword: ret.password});
